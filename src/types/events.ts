...
export const EVENT_TEMPLATES: Record<AuthEventType, (event: AuthEvent) => string> = {
  "user.joined": (event) => {
    ...
    if (event.status === "failed") return `${name} failed to join`;
    return `${name} joined!`;
  },
  "user.updated": (event) => {
    const name = event.metadata?.name || event.metadata?.email || "User";
  ...
  "user.logged_in": (event) => {
    const name = event.metadata?.name || event.metadata?.email || "User";
    if (event.status === "failed") return `${name} failed to login`;
    return `${name} logged in`;
  },
  "user.logged_out": (event) => {
    const name = event.metadata?.name || event.metadata?.email || "User";
    if (event.status === "failed") return `${name} failed to logout`;
    return `${name} logged out`;
  },
  "user.password_changed": (event) => {
    const name = event.metadata?.name || event.metadata?.email || "User";
    if (event.status === "failed") return `${name} failed to change password`;
    return `${name} changed password`;
  },
  "user.email_verified": (event) => {
    const name = event.metadata?.name || event.metadata?.email || "User";
    if (event.status === "failed") return `${name} failed to verify email`;
    return `${name} verified email`;
  },
  "user.banned": (event) => {
    const name = event.metadata?.name || event.metadata?.email || "User";
    if (event.status === "failed") return `${name} failed to ban`;
    return `${name} was banned`;
  },
  "user.unbanned": (event) => {
    const name = event.metadata?.name || event.metadata?.email || "User";
    if (event.status === "failed") return `${name} failed to unban`;
  ...
  "session.created": (event) => {
    const name = event.metadata?.name || event.metadata?.email || "User";
    if (event.status === "failed") return `Failed to create session for "${name}"`;
  ...
  "password.reset_requested": (event) => {
    const email = event.metadata?.email || "User";
    if (event.status === "failed") return `Failed to request password reset for "${email}"`;
  ...
  "password.reset_completed": (event) => {
    ...
    if (event.status === "failed") return `Failed to complete password reset for "${name}"`;
    return `${name} reset their password`;
  },
  "password.reset_requested_otp": (event) => {
    const email = event.metadata?.email || "User";
    if (event.status === "failed") return `Failed to request password reset OTP for "${email}"`;
  ...
  "oauth.linked": (event) => {
    ...
    if (event.status === "failed") return `Failed to link OAuth account "${provider}"`;
    return `OAuth account linked: ${provider}`;
  },
  "oauth.unlinked": (event) => {
    const provider = event.metadata?.provider || "OAuth";
    if (event.status === "failed") return `Failed to unlink OAuth account "${provider}"`;
  ...
  "oauth.sign_in": (event) => {
    ...
    if (event.status === "failed") return `Failed to sign in via ${provider} for "${name}"`;
    return `${name} signed in via ${provider}`;
  },
  "team.created": (event) => {
    const teamName = event.metadata?.teamName || "Team";
    const orgName = event.metadata?.organizationName || "organization";
    if (event.status === "failed") return `Failed to create team "${teamName}" in "${orgName}"`;
    return `Team "${teamName}" created in ${orgName}`;
  },
  "team.updated": (event) => {
    const teamName = event.metadata?.teamName || "Team";
    const orgName = event.metadata?.organizationName || "organization";
    if (event.status === "failed") return `Failed to update team "${teamName}" in "${orgName}"`;
    return `Team "${teamName}" updated in ${orgName}`;
  },
  "team.deleted": (event) => {
    const teamName = event.metadata?.teamName || "Team";
    const orgName = event.metadata?.organizationName || "organization";
    if (event.status === "failed") return `Failed to delete team "${teamName}" from "${orgName}"`;
  ...
  "phone_number.otp_requested": (event) => {
    ...
    if (event.status === "failed") return `Failed to send OTP to ${phoneNumber}`;
    return `Phone number OTP requested for ${phoneNumber}`;
  },
  "phone_number.verification": (event) => {
    const phoneNumber = event.metadata?.phoneNumber || "phone number";
    const name = event.metadata?.name || event.metadata?.email || event.userId || "User";
    if (event.status === "failed") return `Phone number verification failed for ${phoneNumber}`;
