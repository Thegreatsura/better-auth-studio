...
export function createPostgresProvider(options: {
  ...
  const ensureTableSync = async (): Promise<void> => {
    if (tableEnsured) {
      return;
    }
...
export function createNodeSqliteProvider(options: {
  ...
  if (!client) {
    throw new Error(
      "Node SQLite client is required. Provide a DatabaseSync instance from node:sqlite (e.g. new DatabaseSync(':memory:') or new DatabaseSync('path/to/db.sqlite')).",
    );
  }
  ...
  if (!actualClient) {
    throw new Error(
      "Node SQLite client initialization failed. Ensure you pass a node:sqlite DatabaseSync instance (Node.js 22.5+).",
    );
  }
  ...
  if (!hasExec || !hasPrepare) {
    throw new Error(
      "Invalid Node SQLite client. Client must have `exec` and `prepare` methods (node:sqlite DatabaseSync). " +
    ...
    async getStats(): Promise<{
      ...
      try {
        ...
        const failedRow = actualClient
          .prepare(
            `SELECT COUNT(*) as c FROM ${tableName} WHERE status = 'failed' OR display_severity = 'failed'`,
          )
          .get() as { c: number };
        ...
        const infoRow = actualClient
          .prepare(
            `SELECT COUNT(*) as c FROM ${tableName} WHERE display_severity = 'info' OR (display_severity IS NULL AND status != 'failed')`,
          )
          .get() as { c: number };
...
export function createStorageProvider(options: {
  ...
  const isMissingModelError = (error: any) =>
  ...
  const withResolvedModel = async <T>(callback: (modelName: string) => Promise<T>): Promise<T> => {
    const candidates = Array.from(
      new Set([resolvedModelName, ...getModelCandidates()].filter(Boolean) as string[]),
    );
  ...
  const getFieldCandidates = (field?: string) => {
    switch (field) {
      case "userId":
  ...
  const getWhereCandidates = (where?: Array<Record<string, any>>) => {
  ...
  const findManyWithResolvedModel = async (options: Record<string, any>): Promise<any[]> => {
    ...
    for (let index = 0; index < whereCandidates.length; index += 1) {
      const result = (await withResolvedModel((modelName) =>
        adapter.findMany({
          ...restOptions,
    ...
    tableEnsuringPromise = (async () => {
      try {
        if (adapter.findMany) {
          ...
          await withResolvedModel((modelName) =>
            adapter.findMany({
              model: modelName,
              limit: 1,
            }),
